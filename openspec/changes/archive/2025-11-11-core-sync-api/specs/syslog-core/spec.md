...
The system SHALL validate facility names and log levels with TypeScript intellisense support.

#### Scenario: Valid facility names
- **WHEN** accessing facility options via TypeScript
- **THEN** the system SHALL provide autocomplete for all 16 standard facilities
- **INCLUDING** user, local0-7, daemon, mail, auth, etc.

#### Scenario: Invalid log level
- **WHEN** an invalid log level is provided
- **THEN** the system SHALL throw a TypeError
- **AND** list valid level options
